...
SELECT COUNT(oi.product_id) AS count_products, 
	CASE 
...
SELECT 
    TIMESTAMPDIFF(MONTH,
        ...
        MAX(order_purchase_timestamp)) AS count_months
FROM
    orders;
	-- 25 months
...
SELECT 
    COUNT(DISTINCT seller_id) AS count_tech_sellers
FROM
    sellers
        LEFT JOIN
...
SELECT (454 / 3095) * 100 AS percentage_tech_sellers;
	-- 14.67%
...
SELECT 13494400.74/ 3095 / 25 AS avg_month_income_sellers;
	-- 174.40
...
SELECT 1666211.28 / 454 / 25 AS avg_month_income_tech;
	-- 146.80
...
SELECT AVG(DATEDIFF(order_delivered_customer_date, order_purchase_timestamp)) AS avg_time_del
FROM orders;
	-- 12.5035
